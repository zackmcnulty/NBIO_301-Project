...
v_A = v_rest + 5; % at what level does A current activate.

% input current (stimulus) - Give some function I(t)

% step current:
%I_ex =  @(t) 10 * (t >= 25) .* (t <= 75); % normal
%I_ex =  @(t) 25 * (t >= 25) .* (t <= 75); % Ca bursting
% I_ex =  @(t) 10 * (t >= 25); % A current
I_ex =  @(t) 10 * (t >= 25); % A current

x0 = [v_rest 0.3 0.1 0.6 0 0]; % initial conditions; approximated off graph pg 33 FCN
tspan = [0 1000]; % time period to simulate HH model for
...
% 0 V-Ca open at v_rest and all open at 30
 ...
 %1/tau_A *((x(1) > (v_rest)) -  x(6))%
 phi * ( 8*exp((x(1) - v_A) / tau_A) / (1 + exp((x(1) - v_A) / tau_A)).^2.*(1-x(6)) - x(6));
 ];

%    0.05 * ( (x(1) > (v_rest + 3)) - 1/tau_A * x(6) )];

% run the numerical integration of our ODE system
[t_vals, y_vals] = ode45(diff_eqns, tspan, x0);


% sigmoid derivative is e^( (V - V_rest) / tau_A) / (1 + e^((V - V_rest))^2




%% Plotting of Normal HH without A current of Ca gated K+
close all; clc;
...
title("Action Potentials Generated By Hodgkin-Huxley Model");% 
xlim(tspan)
set(gca, 'fontsize', 15);
ylim([-80, 75])
...
ylabel("Current Applied I_{ext}(t)")
...
xlabel("time (ms)");
legend({"V(t)", "threshold", 'V_{rest}'});
title("Low Frequency Action Potentials under Influence of A current K^+ Channels");
xlim(tspan)
set(gca, 'fontsize', 15);
ylim([-80, 75])
% 
...
% plot(t_vals, y_vals(:, 3)), hold on;
plot(t_vals, y_vals(:, 4)), hold on;
plot(t_vals, y_vals(:, 6)), hold on;
plot(t_vals, y_vals(:, 6).^3 .* y_vals(:,4));
title("Gating Variables")
legend({"A-current Inactivation", "A-current Activation", "Proportion Channels Open"});
ylabel("Probability")
xlim(tspan);
ylim([0,1]);
